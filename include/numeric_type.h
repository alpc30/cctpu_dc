...
const HSPS::rational EPS(1,1000);
...
#define INFINITE(x) (!std::isfinite<double>(x))
...
#define CEIL_TO_INT(x) ((long)ceil(x))
#define FRAC(x)  (x - floor(x))
#define FLOOR_TO(x,p) (floor((x) * (p)) / (p))
#define CEIL_TO(x,p) (ceil((x) * (p)) / (p))
#define ABS(x) ((x) < 0 ? -1 * (x) : (x))
#define IS_ZERO(x) (ABS(x) < 1e-11)
#define SAFE_GREATER(x,y) (((x) - (y)) > 1e-11)

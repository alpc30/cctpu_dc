...
index_type graph::extract_path(const index_vec& d, index_type n,
		index_vec& p) const
{
	index_type l = d[n];
	if ((l == no_such_index) || (l <= 1))
	{
		p.clear();
		return l;
	}
...
index_type graph::shortest_path(index_type s, index_type t, index_vec& p) const
{
	index_vec d;
	distance(s, d);
...
index_type graph::shortest_path(const index_set& s, const index_set& t,
		index_vec& p) const
{
	index_vec d;
	distance(s, d);
...
index_type graph::n_induced_undirected_edges(const index_set& n0,
		const index_set& n1) const
{
	index_type n = 0;
	for (index_type i = 0; i < n0.length(); i++)
		for (index_type j = 0; j < n1.length(); j++)
			if (adj[n0[i]][n1[j]] || adj[n1[j]][n0[i]])
				n += 1;
	return n;
}
...
index_type graph::n_bidirectional_edges(const index_set& n0,
		const index_set& n1) const
{
	index_type n = 0;
	for (index_type i = 0; i < n0.length(); i++)
...
bool graph::equals(const graph& g, const index_vec& c) const
{
	...
	if (c.length() != g.size())
	{
		::std::cerr << "error: size of graph " << g
				<< " and permutation vector " << c << " do not agree"
				<< ::std::endl;
		exit(255);
	}
	for (index_type i = 0; i < size(); i++)
...
void graph::difference(const graph& g, const index_vec& c, pair_set& d0,
		pair_set& d1) const
{
	assert(g.size() == size());
...
void graph::difference(const graph& g, pair_set& d0, pair_set& d1) const
{
	assert(g.size() == size());
	d0.clear();
	d1.clear();
...
void graph::add_edge_to_transitive_closure(index_type src, index_type dst,
		pair_set& e)
{
	if (!adj[src][dst])
	{
...
void graph::intersect(const graph& g)
{
	if (g._size != _size)
	{
		::std::cerr << "error: can't intersect " << *this << " of size "
				<< _size << " with graph " << g << " of size" << g._size
				<< ::std::endl;
		exit(255);
	}
	for (index_type i = 0; i < _size; i++)
...
graph& graph::minimal_distance_graph(graph& g, const index_set& s0) const
{
	...
	for (index_type i = 0; i < size(); i++)
		for (index_type j = 0; j < size(); j++)
			if (adjacent(i, j) && (d[i] != no_such_index)
					&& (d[j] == (d[i] + 1)))
				g.add_edge(i, j);
	return g;
}
...
graph& graph::complete_npart(graph& g, const index_set_vec& parts) const
{
	...
	for (index_type p = 0; p < parts.length(); p++)
		for (index_type q = p + 1; q < parts.length(); q++)
		{
			...
			for (index_type ip = 0; (ip < parts[p].length()) && is_complete;
					ip++)
				for (index_type iq = 0; (iq < parts[q].length()) && is_complete;
						iq++)
...
index_type graph::fillin(const index_set& nodes) const
{
	...
	for (index_type i = 0; i < nodes.length(); i++)
		for (index_type j = i + 1; j < nodes.length(); j++)
			if (!bi_adjacent(nodes[i], nodes[j]))
				f += 1;
	return f;
}
...
void graph::max_clique(index_set& sel, index_type next, index_set& clique) const
{
	if (next >= _size)
	{
		if (sel.length() > clique.length())
			clique = sel;
	}
...
void graph::maximal_clique_including(index_type node, index_set& clique) const
{
...
void graph::all_max_cliques(index_set& sel, index_type next,
...
void graph::all_maximal_cliques_including(index_type node,
		index_set_vec& cliques) const
{
	assert(_size > 0);
	cliques.assign_value(EMPTYSET, 1);
...
void graph::scc_first_dfs(index_type n, bool_vec& visited, index_vec& num) const
{
	visited[n] = true;
	for (index_type k = 0; k < _size; k++)
...
void graph::scc_second_dfs(index_type n, bool_vec& visited, index_type c_id)
{
	visited[n] = true;
	comp[n] = c_id;
	for (index_type k = 0; k < _size; k++)
...
void graph::undirected_dfs(index_type n, bool_vec& visited) const
{
	visited[n] = true;
	for (index_type k = 0; k < _size; k++)
...
void graph::strongly_connected_components()
{
	...
	n_comp = 0;
	for (index_type k = num.length(); k > 0; k--)
		if (!visited[num[k - 1]])
		{
...
void graph::apx_independent_set(const index_set& nodes, index_set& set) const
{
	...
	while (!n.empty())
	{
		...
		if (nextI.length() > set.length())
			set.assign_copy(nextI);
		n.subtract(nextC);
	}
}
...
void graph::apx_independent_set_including(index_type node, index_set& set) const
{
	assert((0 <= node) && (node < _size) && (_size > 0));
	set.clear();
...
void graph::all_cliques_geq(index_type k, index_set_vec& cliques) const
{
	cliques.clear();
	if (_size > 0)
	{
...
void graph::all_nondominated_cliques_aux(index_set_vec& cliques,
		index_set& current_clique, const index_set& candidates, index_type min,
	...
	else
	{
		...
		for (index_type i = 0; i < chosen_set.length(); i++)
		{
			...
			if ((current_clique.length() + new_candidates.length() + 1) >= min)
			{
				...
				all_nondominated_cliques_aux(cliques, current_clique,
						new_candidates, min, one_maximal_only);
...
void graph::write_digraph(::std::ostream& s, bool with_node_indices,
...
void graph::write_undirected_graph(::std::ostream& s, bool with_node_indices,
		const char* name) const
{
...
void graph::write_component_labeled_digraph(::std::ostream& s,
		const char* name) const
{
	write_labeled_digraph<index_vec>(s, *this, comp, false, name,
			no_such_index);
}

void graph::write_graph_correspondance(::std::ostream& s, const graph& g,
		const index_vec& c, const char* name) const
{
	assert(g.size() == size());
	assert(c.length() == g.size());
...
void index_graph::write_node_style(std::ostream& s, index_type l)
{
	s << "shape=";
...
void index_graph::write_edge_style(std::ostream& s, index_type l)
{
	if ((l & EDGE_DIR) == ED_FORWARD)
		s << "dir=forward";
	else if ((l & EDGE_DIR) == ED_BACK)
...
void index_graph::write_styled_digraph(std::ostream& s, bool with_node_indices,
		const char* name, index_type c_id) const
{
	...
	for (index_type i = 0; i < size(); i++)
		for (index_type j = 0; j < size(); j++)
			if (adjacent(i, j))
			{
				...
				write_edge_style(s,
						edge_has_label(i, j) ? edge_label(i, j) : 0);
				s << "];" << std::endl;
			}
...
void index_graph::write_matrix(std::ostream& s) const
{
	for (index_type i = 0; i < size(); i++)
	{
		for (index_type j = 0; j < size(); j++)
...
void index_graph::write_MATLAB(std::ostream& s, const char* n,
		const char* t) const
{
	s << "defgraph('";
	if (n)
...
weighted_graph& weighted_graph::quotient(weighted_graph& g,
...
void weighted_graph::increment_edge_weight(index_type src, index_type dst, NTYPE w)
{
...
void weighted_graph::add_undirected_edge(index_type n0, index_type n1, NTYPE w)
{
	graph::add_undirected_edge(n0, n1);
	set_weight(n0, n1, w);
	set_weight(n1, n0, w);
...
void weighted_graph::recursive_tree_decomposition(NTYPE alpha, NTYPE delta, index_set_graph& t) const
{
	if (size() <= 2)
	{
		t.init(1);
...
void weighted_graph::vs_compute_partitions(const index_set& s,
		index_set_vec& p) const
{
...
void weighted_graph::min_and_max_edges(const index_set& nodes, pair_set& e_min,
		NTYPE& w_min,
...
index_set_graph::index_set_graph(const graph& g, const equivalence& eq) :
...
index_set_graph::index_set_graph(const index_set_graph& g,
		const equivalence& eq)
{
	g.quotient(*this, eq);
}

void index_set_graph::add_edge(index_type src, index_type dst)
{
...
void index_set_graph::add_edge(const index_set& srcs, index_type dst)
{
...
void index_set_graph::add_edge(index_type src, const index_set& dsts)
{
		...
		const index_set& n) const
{
	...
	if (i == no_such_index)
		return no_such_index; // graph has no leaf in n
	index_type i_max = i;
...
index_set_graph& index_set_graph::quotient(index_set_graph& g,
		const equivalence& eq) const
{
	graph::quotient(g, eq);
	g.clear_node_labels();
...
index_set_graph& index_set_graph::edge_label_quotient(index_set_graph& g,
		const equivalence& eq) const
{
	graph::quotient(g, eq);
	g.clear_node_labels();
...
index_set_graph& index_set_graph::subgraph_set_size_gt(index_set_graph& g,
		index_type l)
{
	index_set r;
	for (index_type k = 0; k < size(); k++)
...
void index_set_graph::write_DOT(std::ostream& s, bool undirected,
		const char* name) const
{
	if (undirected)
		s << "graph";
